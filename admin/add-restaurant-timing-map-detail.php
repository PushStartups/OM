...
<script>
    $(document).ready(function() {
        ...
        for(var i=1;i<3;i++){
            ...
            div.className = "map"+(i);
            div.id = "map"+(i);
            div.style.width = ($('#holder').width() / 2) + "px";
...
<script>
    ...
    function initialize() {
        ...
        <?php
        $latlng1 = DB::query("select * from delivery_info where restaurant_id = '$restaurant_id'");
        $c = 1;
        $info_id = Array();
        foreach($latlng1 as $l)
        {
        ...
        $latlng = DB::query("select * from delivery_info_detail where delivery_info_id = '".$info_id[$i]."'");
